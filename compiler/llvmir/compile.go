...
func compileBlock(ctx *Context, code ast.BlockStmt) {
	for _, stmt := range code.Stmts {
		...
		switch v := stmt.(type) {
		case ast.FuncCall:
			// FIXME: Move this into a method to share with evalValue
			...
			for i, arg := range v.UserArgs {
				...
				case "PrintInt":
					...
					val := evalValue(ctx, output, arg, false, nil)
				...
				case "PrintString", "PrintByteSlice":
					...
					val := evalValue(ctx, output, arg, false, ast.TypeLiteral("uint8"))
					args = append(args, val)
				case "Write":
					val := evalValue(ctx, output, arg, false, nil)
					args = append(args, val)
				case "len":
					panic("Naked call to len in block")
				default:
					switch t := argdefs[i].Type().(type) {
					case ast.ArrayType:
						base := evalValue(ctx, output, arg, true, nil)
						args = append(args, base)
					case ast.SliceType:
						base := evalValue(ctx, output, arg, false, nil)
						args = append(args, base)
					case ast.SumType:
						base := evalValue(ctx, output, arg, false, t)
						args = append(args, base)
					default:
						if argdefs[i].Reference {
							args = append(args, evalValue(ctx, output, arg, true, nil))
						} else {
							args = append(args, evalValue(ctx, output, arg, false, nil))
						}
					}
				}
			}
			output.NewCall(fnc.Function, args...)
		case ast.ReturnStmt:
			if v.Val == nil {
				output.NewRet(nil)
			} else {
				rett := ctx.curfuncdef.Return[0].Type()
				rv := evalValue(ctx, output, v.Val, false, rett)
				output.NewRet(rv)
			}
		case ast.LetStmt:
		...
		case ast.MutStmt:
			...
			val := evalValue(ctx, output, v.InitialValue, false, t)
		...
		case ast.AssignmentOperator:
			...
			case ast.VarWithType:
				...
				val := evalValue(ctx, ctx.curblock, v.Value, false, nil)
			...
			case ast.ArrayValue:
				...
				ptr := evalValue(ctx, ctx.curblock, vr, true, nil)
				val := evalValue(ctx, ctx.curblock, v.Value, false, t)
		...
		case ast.WhileLoop:
			...
			// The condition conditionally branches to either the end or the body

			//ocb := ctx.curblock
			// Declare alloca and initialize them for any let conditionals
			ctx.curblock = loopinit
			initializeCondition(ctx, loopinit, v.Condition)

			// To avoid executing the let stmts twice, loopinit checks the condition too
			val := evalValue(ctx, ctx.curblock, v.Condition, true, nil)
			...
			val2 := evalValue(ctx, ctx.curblock, v.Condition, true, nil)
		...
		case ast.IfStmt:
			...
			ctx.curblock = ifcond
			val := evalValue(ctx, ctx.curblock, v.Condition, false, nil)
				...
				case ast.EnumTypeDefn:
					...
					if len(e.Parameters) == 0 {
						...
						for i, c := range v.Cases {
							...
							v2 := evalValue(ctx, ctx.curblock, c.Variable, false, nil)
...
func evalValue(ctx *Context, fnc *ir.BasicBlock, val ast.Value, addronly bool, arrayType ast.Type) value.Value {
	...
	case ast.SumType:
		...
		literal := evalValue(ctx, fnc, val, addronly, nil)
		ltype := literal.Type()
	...
	case ast.EnumValue:
		...
		for _, v := range v.Parameters {
			subpiece := evalValue(ctx, fnc, v, false, v.Type())
	...
	case ast.FuncCall:
		if v.Name == "len" {
			v := evalValue(ctx, ctx.curblock, v.UserArgs[0], false, ast.TypeLiteral("uint64"))
		...
		argdefs := fnc.FuncDecl.Args
		for i, arg := range v.UserArgs {
			var deref bool = argdefs[i].Reference
			if _, ok := argdefs[i].Type().(ast.ArrayType); ok {
				deref = true
			}
			v := evalValue(ctx, ctx.curblock, arg, deref, argdefs[i].Type())
			args = append(args, v)
		}
		return ctx.curblock.NewCall(fnc.Function, args...)
	case ast.SubtractionOperator:
		l := evalValue(ctx, ctx.curblock, v.Left, false, arrayType)
		r := evalValue(ctx, ctx.curblock, v.Right, false, arrayType)
	...
	case ast.ArrayLiteral:
		...
		for _, e := range v {
			sv := evalValue(ctx, fnc, e, false, arrayType)
			switch val := sv.(type) {
			case *constant.Int:
				arr = append(arr, val)
	...
	case ast.ArrayValue:
		...
		case ast.ArrayType:
			idx := evalValue(ctx, fnc, v.Index, false, arrayType)
	...
	case ast.Cast:
		irv := evalValue(ctx, fnc, v.Val, addronly, arrayType)
		if len(v.Val.Type().Components()) == 1 {
			return ctx.convValue(irv, v.Val.Type(), v.Typ)
		}
		return irv
	case ast.Brackets:
		return evalValue(ctx, fnc, v.Val, addronly, arrayType)
	case ast.LetStmt:
		...
		// will work
		loc := ctx.GetVariable(v.Var)
		val := evalValue(ctx, fnc, v.Val, false, v.Var.Type())
		if loc.FromMemory {
	...
	case ast.TupleValue:
		...
		for i, aval := range v {
			val := evalValue(ctx, fnc, aval, false, ty[i].Type())
		...
		return rv
	case ast.Slice:
		base := evalValue(ctx, fnc, v.Base, true, arrayType)
...
func initializeCondition(ctx *Context, block *ir.BasicBlock, condition ast.Value) {
	switch c := condition.(type) {
	case ast.EqualityComparison:
		initializeCondition(ctx, block, c.Left)
		initializeCondition(ctx, block, c.Right)
	case ast.NotEqualsComparison:
		initializeCondition(ctx, block, c.Left)
		initializeCondition(ctx, block, c.Right)
	case ast.LessThanComparison:
	...
	case ast.GreaterOrEqualComparison:
		...
		initializeCondition(ctx, block, c.Right)
	case ast.Brackets:
		initializeCondition(ctx, block, c.Val)
	case ast.LetStmt:
		loc := block.NewAlloca(getType(c.Var.Type()))
		ctx.curblock = block
		//val := evalValue(ctx, block, c.Val, false, nil)
		evalValue(ctx, block, c.Val, false, nil)
		ctx.SetVar(c.Var, Register{
			Value:      loc,
			FromMemory: true,
		})
	case ast.IntLiteral, ast.VarWithType, ast.BoolLiteral:
		// Nothing
	default:
		panic(fmt.Sprintf("Unhandled type in condition initializer: %v", reflect.TypeOf(c)))
	}
}
...
func initializeLetStmt(ctx *Context, v ast.LetStmt) {
	...
	case ast.SliceType:
		val := evalValue(ctx, ctx.curblock, v.Val, false, t.Base)
		var finalval value.Value
		switch v := v.Val.(type) {
		case ast.ArrayLiteral:
			// If it's an array literal, get the base from element 0
	...
	case ast.TupleType:
		val := evalValue(ctx, ctx.curblock, v.Val, false, t)
		ctx.SetVar(v.Var, Register{
			Value:      val,
			FromMemory: false,
		})
